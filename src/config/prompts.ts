...
const TOOL_INSTRUCTIONS: Record<string, string> = {
  ...
  run_command:
    "Use run_command to act on the user's computer and to read it. It is the tool for any question about their own machine that a command can answer — free disk space, memory, what is running, the files in a folder, their IP address, uptime, a git status — as well as for opening an application, running a program, or looking something up in their browser. A registered name of theirs or the real command both work, and arguments go after the name (`vscode ~/notes.md`, `search tallest mountain`). A command they have not already allowed is shown to them for approval first. When the result includes output, answer from that text: you can read their machine through this tool. Never reply that you cannot access their files, their computer, or their system, and never tell them to run it themselves — you can, through this tool, so run it. If it reports that a program is not installed, retry once with one of the registered names it lists. If the user declines or the command is refused, say so plainly and do not propose it again.",
  get_snippet:
